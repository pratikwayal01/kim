...
def _load_oneshots_raw():
    """Return the raw list from oneshots.json (including expired), or []."""
    if not ONESHOT_FILE.exists():
        return []
    try:
        return json.loads(ONESHOT_FILE.read_text(encoding="utf-8"))
    except (json.JSONDecodeError, OSError):
        return []


def _save_oneshots(oneshots: list) -> None:
    """Atomically overwrite oneshots.json."""
    try:
        ...
        print(f"Error writing {ONESHOT_FILE}: {e}")
        sys.exit(1)


def _cmd_remind_list():
    """Print pending one-shot reminders."""
    import datetime as _dt

    oneshots = _load_oneshots_raw()
    now = time.time()
    pending = [o for o in oneshots if o.get("fire_at", 0) > now]

    if not pending:
        print("No pending one-shot reminders.")
        return

    pending.sort(key=lambda o: o["fire_at"])
    print(f"{'#':<4} {'MESSAGE':<30} {'FIRES AT':<20} {'IN'}")
    print("-" * 70)
    for i, o in enumerate(pending, 1):
        msg = o.get("message", "")[:28]
        fire_dt = _dt.datetime.fromtimestamp(o["fire_at"]).strftime("%Y-%m-%d %H:%M")
...
def _cmd_remind_cancel(token: str):
    """
    Cancel a pending one-shot by 1-based index (from --list) or message substring.
    """
    oneshots = _load_oneshots_raw()
    now = time.time()
    ...
    target_idx = None  # index into oneshots[]
    ...
    if token.isdigit():
        ...
        if 1 <= n <= len(pending_indices):
            ...
            print(
                f"No reminder at index {n}. Run 'kim remind --list' to see pending reminders."
            )
            sys.exit(1)
    else:
        # Match by message substring (case-insensitive, first match)
        ...
        if target_idx is None:
            print(f"No pending reminder matching '{token}'.")
            sys.exit(1)
    ...
    print(f"Cancelled: '{removed.get('message', '')}' (was due at {fire_dt})")
    log.info("One-shot reminder cancelled: '%s'", removed.get("message", ""))


def cmd_remind(args):
    # --list: show pending one-shots
    if getattr(args, "list_oneshots", False):
        _cmd_remind_list()
        return

    # --cancel: remove a one-shot by 1-based index or message substring
    cancel = getattr(args, "cancel", None)
    if cancel is not None:
        _cmd_remind_cancel(cancel)
        return

    # Normal usage: require message and time
    if not args.message or not args.time:
        print('Usage: kim remind <message> <time>  (e.g. kim remind "standup" in 10m)')
        print("       kim remind --list")
        print("       kim remind --cancel <index|message>")
        sys.exit(1)
